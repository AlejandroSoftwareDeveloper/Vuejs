...
onMounted(async () => {
  try {
    ...
    salary.value = tmp.salary;
    location.value = tmp.location;
    name.value = tmp.company.name;
    cdescription.value = tmp.company.description;
    contactEmail.value = tmp.company.contactEmail;
    contactPhone.value = tmp.company.contactPhone;
  } catch (error) {
    ...
    <div class="container max-w-2xl py-24 m-auto">
      <div class="px-6 py-8 m-4 mb-4 bg-white border rounded-md shadow-md md:m-0">
        <form @submit.prevent="saveData">
          <h2 class="mb-6 text-3xl font-semibold text-center">Editar trabajo</h2>
          ...
          <InputBlock labeltext="Tipo de trabajos">
            <input type="text" id="title" name="title" class="w-full px-3 py-2 mb-2 border rounded "
              placeholder="Ejemplo: Desarrollador frontend" required v-model="title" />
          ...
          <InputBlock labeltext="Descripcion del trabajo">
            <textarea id="description" name="description" class="w-full px-3 py-2 border rounded" rows="4"
              placeholder="Adicione cualquier tarea, expectativas, requisitos, etc" v-model='description'></textarea>
            <template v-slot:error>
          ...
          <InputBlock labeltext="Nombre de la compania">
            <input type="text" id="name" name="name" class="w-full px-3 py-2 border rounded"
              placeholder="Nombre de la compania" v-model="name" />
            <template v-slot:error>
          ...
          <InputBlock labeltext="Descripcion de la compania">
            <textarea id="company_description" name="company_description" class="w-full px-3 py-2 border rounded"
              rows="4" placeholder="Que hace su compania" v-model="cdescription"></textarea>
            <template v-slot:error>
          ...
          <InputBlock labeltext="Direccion">
            <input type="text" id="location" name="location" class="w-full px-3 py-2 mb-2 border rounded"
              placeholder="Direccion de la compania" required v-model="location" />
            <template v-slot:error>
          ...
          <InputBlock labeltext="Email de la compania">
            <input type="email" id="contact_email" name="contact_email" class="w-full px-3 py-2 border rounded"
              placeholder="Direccion de correo electronico para los solicitantes" required v-model="contactEmail" />
            <template v-slot:error>
          ...
          <InputBlock labeltext="Telefono de la compania">
            <input type="text" id="contact_phone" name="contact_phone" class="w-full px-3 py-2 border rounded"
              placeholder="Telefono para los solicitantes" v-model="contactPhone" />
            <template v-slot:error>
